...
class ExecutionEnv:
    ...
    def create(cls, offload_dir):
        ...
        from h2o_flexgen.flexgen.pytorch_backend_bk import TorchDevice, TorchDisk, TorchMixedDevice
...
class Dataset():
    def __init__(self, 
        ...
        # make dataset
        if not self.target_path: # target_path为none，即 answer在data中 
            ...
            if self.data_path.endswith('.jsonl'):
                ...
                raise ValueError('错误的数据集文件格式')
            if self.target_path.endswith('.lst'):
