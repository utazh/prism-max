...
from pytorch_backend_bk import (TorchDevice, TorchDisk, TorchLink,
...
DEVICE_MOVE_TIME_PER_CHUNK = {"disk": 0.0045, "cpu":0.001, "gpu":0} # seconds
TIME_BUDGET_PER_LAYER = 0.01
PREFETCH_RATIO = None
...
class SelfAttention:
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        output_dir = "./time_logs"
        os.makedirs(output_dir, exist_ok=True)
        output_file = os.path.join(output_dir, f"temp_trash.txt")

        if i == 0:  # prefill
            mask, donate[1] = attention_mask.val.smart_copy(self.compute)
            # print(f'mask = {mask.shape} {mask.data}') # shape (b, s), True if not padding
            # print(f'input: {self.task.inputs[0]}')
            global pcache,dt_prefix,dt_all,dt_load,dt_sele,dt_load_head,dt_suffix,dt_key,dt_value
            if self.policy.prefix_aware_inf:
                ...
                if self.policy.full_load:
                    ...
                    if True:
                        ...
                        if self.policy.sele_inf:
                            ...
                            per_layer_compute_time = time.monotonic() - st
                    ...
                    # with open(output_file, 'a') as f:
                    #     f.write(f'Layer {self.layer_id} load head time = {per_layer_dt_key:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} sele token time = {per_layer_dt_sele_head:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} load time = {per_layer_dt_load_value:.8f}s\n')
                    #     # f.write(f'Layer {self.layer_id} load value time = {time.monotonic() - st_value:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} prefetch time = {0:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} compute time = {per_layer_dt_compute_time:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} parallel time = {per_layer_dt_compute_time:.8f}s\n')
                    #     f.write(f'===========================================================================\n')
                else:
                    # the paper's method
                    # sele_load prefix kv
                    ...
                    if True:
                        ...
                        # 每层选指定百分比的 token
                            ...
                            if not self.policy.no_prefetch and self.layer_id < self.config.num_hidden_layers - 1:
                                if self.policy.solid_ratio_prefetch:
                                    ...
                                    initial_prefetch_tokenids = sele_tokenids[:prefetch_token_num]
                                    current_layer = pcache.cache[prefix_table[prefix_hash]].layers[self.layer_id]
                                    prefetch_chunkids = initial_prefetch_tokenids // current_layer.chunk_size
                                    
                                    # next_layer = pcache.cache[prefix_table[prefix_hash]].layers[self.layer_id + 1]
                                    # prefetch_token_num = next_layer.chunk_num - 1
                                    # prefetch_tokenids_list = []
                                    # for i in range(0, prefetch_token_num):
                                    #     for j in range(0, 10):
                                    #         prefetch_tokenids_list.append( i * 64 + j )
                                    # prefetch_tokenids = torch.tensor(prefetch_tokenids_list)
                                else:
                                    # 按照当前层的比例预取
                                    prefetch_token_num = int(len(sele_tokenids) * CURRENT_SAMPLE_PREFETCH_RATIO['layers'][self.layer_id] / cur_sele_percent)
                                    prefetch_tokenids = sele_tokenids[:prefetch_token_num]

                            # if self.layer_id == 0:
                            #     # 第0层使用固定比例预取，目的是profile计算时间
                            #     prefetch_token_num = int(len(sele_tokenids) * self.policy.prefetch_ratio / cur_sele_percent)
                            #     # print(f"prefetch_token_num: {prefetch_token_num}")
                            #     prefetch_tokenids = sele_tokenids[:prefetch_token_num]
                            # elif self.layer_id < self.config.num_hidden_layers - 1:
                            #     # 测试，使用随机长度，为了profile准确
                            #     # prefetch_token_num = np.random.randint(low=1, high=len(sele_tokenids) // 2)
                            #     # prefetch_tokenids = torch.arange(prefetch_token_num, device=sele_tokenids.device)
                            #     prefetch_token_num = int(len(sele_tokenids) * self.policy.prefetch_ratio / cur_sele_percent)
                            #     # prefetch_token_num = 0
                            #     # print(f"prefetch_token_num: {prefetch_token_num}")
                            #     prefetch_tokenids = sele_tokenids[:prefetch_token_num]

                                # 后续层使用时间预算来决定预取多少
                                # next_layer = pcache.cache[prefix_table[prefix_hash]].layers[self.layer_id + 1]
                                # k_sele_chunkids = sele_tokenids // next_layer.chunk_size
                                # # v_sele_chunkids = k_sele_chunkids + next_layer.chunk_num
                                # unique_k = torch.unique(k_sele_chunkids, sorted=False)
                                # unique_kv = torch.cat([unique_k, unique_k + next_layer.chunk_num])
                                # device_ids = next_layer.device_map[unique_kv.cpu().numpy()]
                                # global DEVICE_MOVE_TIME_PER_CHUNK, TIME_BUDGET_PER_LAYER
                                # chunkids_move_time = np.array([DEVICE_MOVE_TIME_PER_CHUNK[d] for d in device_ids])
                                # KV_move_time = chunkids_move_time[:len(chunkids_move_time)//2] + chunkids_move_time[len(chunkids_move_time)//2:]
                                # # unique_v = unique_k + next_layer.chunk_num
                                # move_time_sum = 0
                                # i = 0
                                # for value in KV_move_time:
                                #     # if next_layer.device_map[value.item()] != next_layer.device_map[value.item() + next_layer.chunk_num]:
                                #     #     print(f'Warning!!! Layer {self.layer_id} sele load k/v not in the same device!!!\n')
                                #     move_time_sum += value
                                #     if move_time_sum > TIME_BUDGET_PER_LAYER:
                                #         break
                                #     else:
                                #         i += 1
                                # prefetch_chunkids = unique_k[:i]

                            # sele_tokenids, del_tokenids = self.compute.sele_tokenid(h, mask, w_q, b_q,
                            #             w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head, donate,
                            #             self.policy.sele_head, fullhead_prefix_k, -1, self.policy.sim_thred, cur_sele_percent)
                            # print('sele_token',sele_tokenids, sele_tokenids.shape)
                            # print('prefetch_token',prefetch_tokenids, prefetch_tokenids.shape)
                            #  记录选择的 tokenid
                            # output_dir = "./token_logs"
                            # os.makedirs(output_dir, exist_ok=True)
                            # output_file = os.path.join(output_dir, f"sele_load_openbookqa_selected_tokens.txt")
                            # with open(output_file, 'a') as f:
                            #     f.write(f"Selected Tokens per Head: {sele_tokenids.shape}\n")
                            #     tokens_str = ','.join(map(str, sele_tokenids.cpu().numpy().tolist()))
                            #     f.write(f"Head 0: {tokens_str}\n\n")
                        ...
                        if not self.layer_id == 0 and not self.policy.no_prefetch:
                            # 第0层没有之前的预取结果，只有后面层需要做
                            # 将选取出来的 sele_tokenids 和 self.parent_model.last_layer_prefetched_tokenids 做差集，找到还没有拿到的 tokenids
                            prefetch_mask = ~torch.isin(sele_tokenids, self.parent_model.last_layer_prefetched_tokenids)
                        ...
                        #     device = sele_tokenids.device  # 获取 sele_tokenids 的设备
                        #     already_prefetched_tokenids = already_prefetched_tokenids.to(device)
                        #     # 需要但是还没有被取上来的token
                        #     masked_not_prefetched = ~torch.isin(sele_tokenids, already_prefetched_tokenids)
                        #     additional_tokenids = sele_tokenids[masked_not_prefetched]
                        
                        #     # 找出预取的token中在当前层需要的部分
                        #     mask_needed_prefetched = torch.isin(already_prefetched_tokenids, sele_tokenids)
                        #     needed_prefetched_tokenids = already_prefetched_tokenids[mask_needed_prefetched]
                        #     needed_prefetched_k = already_prefetched_k[mask_needed_prefetched] if already_prefetched_k is not None else None
                        #     needed_prefetched_v = already_prefetched_v[mask_needed_prefetched] if already_prefetched_v is not None else None

                        #     print(f"Layer {self.layer_id}: 总需要{len(sele_tokenids)}个token，"
                        #           f"可复用预取{len(needed_prefetched_tokenids)}个，"
                        #           f"需额外加载{len(additional_tokenids)}个")
                        # else:
                        #     additional_tokenids = sele_tokenids
                        #     needed_prefetched_tokenids = torch.tensor([], device="cuda:0")
                        #     needed_prefetched_k = None
                        #     needed_prefetched_v = None
                        #     print(f"Layer {self.layer_id}: 无预取数据，需加载{len(additional_tokenids)}个")
                        # torch.cuda.synchronize()
                        # st = time.monotonic()
                        ...
                        # with open(output_file, 'a') as f:
                        #     f.write(f'Layer {self.layer_id} load time = {per_layer_dt_load:.8f}s\n')
                            # print(f'load time for layer {self.layer_id} = {per_layer_dt_load:.4f}s')
                        # current_layer = pcache.cache[prefix_table[prefix_hash]].layers[self.layer_id]
                        # print(f'layer_id = {self.layer_id} current_layer.device_map = {current_layer.device_map}')
                        # print(f"prefetch_tokenids = {prefetch_tokenids}")
                        # output_device_file = "./token_device_logs/prefetch_update_varification.txt"
                        # self.print_prefetch_tokenids_device(prefetch_tokenids, output_device_file, prefix_hash)
                        ...
                        # with open(output_file, 'a') as f:
                        #     f.write(f'Layer {self.layer_id} prefetch time = {per_layer_dt_prefetch:.8f}s\n')
                        # self.print_prefetch_tokenids_device(prefetch_tokenids, output_device_file, prefix_hash)
                        # self.print_prefetch_tokenids_device(prefetch_tokenids, "./prefetch_profile/current_bin.txt", prefix_hash)
                        ...
                        with torch.cuda.stream(c_stream):
                            ...
                            if self.policy.suffix_comp:
                                ...
                                # torch.cuda.synchronize()
                                # per_layer_dt_all = time.monotonic() - st
                                # dt_all += per_layer_dt_all
                                # dt_all += time.monotonic() - st
                                # print(f'comp without prefix time for layer {self.layer_id} = {per_layer_dt_all:.4f}s')
                        ...
                        # 先提交计算任务，后提交 prefetch 任务
                        with torch.cuda.stream(p_stream):
                            # p_stream_st = time.monotonic()
                            p_stream_start_event.record()
                            if self.layer_id < self.config.num_hidden_layers - 1 and not self.policy.no_prefetch:
                                if len(prefetch_tokenids) > 0 and not args.no_cache:
                                    # 移动下一层的相同索引的 token 到 gpu
                                    self.parent_model.last_layer_prefetched_prefix_k, self.parent_model.last_layer_prefetched_prefix_v = pcache.get(
                                        prefix_id=prefix_table[prefix_hash],pos_id=prefetch_tokenids,layer=self.layer_id + 1,is_prefetching=True)
                                else:
                                    prefetch_mmap_kv_tensor = np.load(prefetch_prefix_kv_path, mmap_mode='r')
                                    prefetch_k, prefetch_v = torch.from_numpy(prefetch_mmap_kv_tensor[0, prefetch_tokenids, :, :]).cuda(), torch.from_numpy(prefetch_mmap_kv_tensor[1, prefetch_tokenids, :, :]).cuda()
                            # p_stream.synchronize()
                            # prefetch_time = time.monotonic() - p_stream_st
                            p_stream_end_event.record()
                        ...
                        # 预取逻辑
                        if self.layer_id < self.config.num_hidden_layers - 1 and not self.policy.no_prefetch:
                            # 最后，更新记录的预取 tokenids，只能在这里做，因为上面是并行的逻辑
                            self.parent_model.last_layer_prefetched_tokenids = prefetch_tokenids

                            # with open("prefetch_profile/get_token_profile.txt", 'a') as f:
                            #     f.write(f"get token time: {prefetch_time:.8f} s, token_id len: {len(prefetch_tokenids)}, device: {key_device}\n")
                            # print(f"Layer {self.layer_id}: prefetched {len(prefetch_tokenids)} tokens: {prefetch_tokenids}")
                        # self.print_prefetch_tokenids_device(prefetch_tokenids, output_device_file, prefix_hash)
                        # load_time = load_start_event.elapsed_time(load_end_event) / 1000
                        # parallel_time = parallel_start_event.elapsed_time(parallel_end_event) / 1000
                        parallel_time = ed - st
                        prefetch_time = p_stream_start_event.elapsed_time(p_stream_end_event) / 1000
                        ...
                        # prefetch_time = prefetch_start_event.elapsed_time(prefetch_end_event) / 1000
                        # prefetch_time = 0
                        ...
                        with open(output_file, 'a') as f:
                            ...
                            f.write(f'Layer {self.layer_id} sele token time = {per_layer_dt_sele_head:.8f}s\n')
                            f.write(f'Layer {self.layer_id} load time = {per_layer_dt_load:.8f}s\n')
                            f.write(f'Layer {self.layer_id} prefetch time = {prefetch_time:.8f}s\n')
                            f.write(f'Layer {self.layer_id} compute time = {compute_time:.8f}s\n')
                            f.write(f'Layer {self.layer_id} parallel time = {parallel_time:.8f}s\n')
                            f.write(f'===========================================================================\n')

                        # with open("./prefetch_profile/current_bin.txt", 'a') as f:
                        #     f.write(f'Layer {self.layer_id} prefetch time = {prefetch_time:.8f}s\n')
                        #     f.write(f'Layer {self.layer_id} num chunks = {num_chunks * 2:d}\n')
                        #     f.write(f'===========================================================================\n')
                        # print(f'layer_id = {self.layer_id} current_layer.device_map = {current_layer.device_map}')
                ...
                if self.policy.ret_topk_indices:
                    ...
                    dt_all += dt_per_layer
                    # dt_all  += time.monotonic() - st
                    # with open('./time_logs/temp_trash.txt','a') as f:
                    #     f.write(f'Layer {self.layer_id} load head time = {0:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} sele token time = {0:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} load time = {0:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} prefetch time = {0:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} compute time = {dt_per_layer:.8f}s\n')
                    #     f.write(f'Layer {self.layer_id} parallel time = {dt_per_layer:.8f}s\n')
                    #     f.write(f'===========================================================================\n')
    ...
    def generation_loop_normal(self):
        try:
            ...
            for i in range(self.execute_gen_len): # 生成第 i 个token
                ...
                # prefix dump
                if i==0 and self.policy.prefix_dump and not args.recompute:
...
def get_filename(args):
    ...
    if args.input_path:
        ...
        filename += f"fo-{model_size}-" \
                f"{datasetname}-padding_mul{args.padding_mul}-" \
...
def load_tokenizer(args):
    cache_dir = args.hf_cache_dir or os.environ.get("HYPERINFER_HF_CACHE") or os.environ.get("HF_HOME")

    common_kwargs = {"padding_side": "left"}
    if cache_dir:
        common_kwargs["cache_dir"] = cache_dir

    if args.model == "facebook/galactica-30b":
        return AutoTokenizer.from_pretrained("facebook/galactica-30b", **common_kwargs)
    if 'llama' in args.model:
        tokenizer = AutoTokenizer.from_pretrained(args.model, padding="max_length", **common_kwargs)
        tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        tokenizer.pad_token_id = 2
        return tokenizer

    return AutoTokenizer.from_pretrained(args.model, use_fast=True, **common_kwargs)
...
def run_flexgen(args, logtrace_filename):
    ...
    try:
        ...
        if logits_bool and not args.perplexity:
            ...
            # Write JSONL results for evaluate_task_result.py.
    ...
    global dt_prefix,dt_all,dt_load,dt_sele,dt_load_head,dt_suffix,dt_tol,dt_key,dt_value,all_ttft
...
def add_parser_arguments(parser):
    ...
    parser.add_argument("--model", type=str, default="facebook/opt-2.7b",
        help="The model name.")
    parser.add_argument("--path", type=str, default=os.environ.get("WEIGHT_PATH", "~/opt_weights"),
        help="The path to the model weights. If there are no cached weights, "
             "FlexGen will automatically download them from HuggingFace.")
    parser.add_argument("--offload-dir", type=str, default=os.environ.get("OFFLOAD_DIR", "~/flexgen_offload_dir"),
        help="The directory to offload tensors. ")
    parser.add_argument("--hf-cache-dir", type=str,
        default=os.environ.get("HYPERINFER_HF_CACHE") or os.environ.get("HF_HOME"),
        help="Optional HuggingFace cache directory for tokenizers and model downloads.")
