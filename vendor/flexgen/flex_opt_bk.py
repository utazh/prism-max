            ...
            if self.policy.prefix_aware_inf:
                ...
                if self.policy.full_load:
                    ...
                    if True:
                            ...
                            with torch.cuda.stream(p_stream):
                                ...
                                if self.layer_id < self.config.num_hidden_layers - 1 and not self.policy.no_prefetch:
                                    if len(prefetch_tokenids) > 0 and not args.no_cache:
                                        ...
                                        # print(f"len(prefetch_tokenids) = {len(prefetch_tokenids)}")
                                        self.parent_model.last_layer_prefetched_prefix_k, self.parent_model.last_layer_prefetched_prefix_v, prefetch_tokenids = pcache.prefetch(
                                            prefix_id=prefix_table[prefix_hash],pos_id=prefetch_tokenids,layer=self.layer_id + 1,k_v_num_tokens=k_v_num_tokens,time_budget=self.parent_model.prefetch_time_budget)
                                        print(f"prefetched {len(prefetch_tokenids)} tokens for layer {self.layer_id + 1}")
                                    else:
                        ...
                        all_ttft.append(per_layer_dt_load_head + per_layer_dt_sele_head + per_layer_dt_load + parallel_time)
...
def run_flexgen(args, logtrace_filename):
    ...
    if args.model == "facebook/galactica-30b":
        ...
        tokenizer = AutoTokenizer.from_pretrained(f'/data/zrd/huggingface/hub/models--facebook--opt-30b/snapshots/ceea0a90ac0f6fae7c2c34bcb40477438c152546/', padding_side="left", use_fast=True)
