        ...
        if self.key_exist:
            if head is None:
                if pos_id is None:
                    ...
                    keys = keys[pos_id].to(device)
            else:
                if pos_id is None:
                    keys = keys[:,head].to(device)
                else:
                    keys = keys[pos_id,head].to(device)
        if self.value_exist:
            if head is None:
                if pos_id is None:
                    values = values.to(device)
                else:
                    values = values[pos_id].to(device)
            else:
                if pos_id is None:
                    values = values[:,head].to(device)
                else:
                    values = values[pos_id,head].to(device)
        return keys,values
    ...
    def get_key(self,pos_id,device='cuda:0',head=None, is_prefetching=False):
        ...
        if head is None:
            if pos_id is None:
                ...
                keys = keys[pos_id].to(device, non_blocking=True)
        else:
            if pos_id is None:
                keys = keys[:,head].to(device, non_blocking=True)
            else:
                keys = keys[pos_id,head].to(device, non_blocking=True)
        return keys
    ...
    def get_value(self,pos_id,device='cuda:0',head=None, is_prefetching=False):
        ...
        if head is None:
            if pos_id is None:
                ...
                values = values[pos_id].to(device, non_blocking=True)
        else:
            if pos_id is None:
                values = values[:,head].to(device, non_blocking=True)
            else:
                values = values[pos_id,head].to(device, non_blocking=True)
        return values
...
class PrefixKVLayer():
    ...
    def prefetch_by_chunk(self,pos_id=None,need_key=True,need_value=True,k_v_num_tokens=None,time_budget=0.0):
        ...
        if len(pos_id.shape) == 1:
            ...
            pos_id = self.reorder[pos_id] # 变成实际的 token id
            ...
            if time_budget > 0.0:
                ...
                for chunk_id in chunk_ids:
                    ...
                    if need_key:
                        est_time += AVG_CHUNK_TIME[device_key]
                    if need_value:
                        est_time += AVG_CHUNK_TIME[device_value]
                    if time_budget - est_time >= 0.0:
                        time_budget -= est_time
                    else:
                        chunk_ids = chunk_ids[chunk_ids < chunk_id] # 截断在当前 chunk 之前的所有 chunk，利用了 unique 的排序副作用
                        break
            else:
                # 固定比例预取，取一半的 chunk
            ...
            start_indices = (chunk_ids * self.chunk_size).unsqueeze(1)
            offsets = torch.arange(self.chunk_size, device=start_indices.device)

            # 使用广播将偏移量加到每个起始位置上
            # (N, 1) + (C,) -> (N, C)
            # 每一行都是一个 chunk 对应的 token_ids 范围
            token_ranges = start_indices + offsets
            candidate_tokenids = token_ranges.flatten()
            # print(f"Layer {self.layer_id} candidate_tokenids before valid filter: {candidate_tokenids.cpu().numpy()}")
            valid_mask = candidate_tokenids < k_v_num_tokens
            prefetch_tokenids = candidate_tokenids[valid_mask] # 拓展后的实际存储 id
            token_ids = prefetch_tokenids % self.chunk_size # chunk 内的 token id
            # print(f"Layer {self.layer_id} prefetch_tokenids after valid filter: {prefetch_tokenids.cpu().numpy()}")
            # print(f"Layer {self.layer_id} self.reverse_reorder: {self.reverse_reorder}")
            ...
            for chunk_id in chunk_ids:
                mask = (prefetch_tokenids // self.chunk_size == chunk_id )
                # mask is used to select the tokens in the current chunk
                # if not in the current chunk, mask will be false, and this token will not be selected
                token_id = token_ids[mask]
            if self.disk_type == 'KV_Division':
                if need_key:
                    ...
                    # torch.cuda.synchronize()
                    # get_key_st = time.monotonic()
                    key_buffer[mask] = chunk.get_key(token_id,is_prefetching=True)
                    # torch.cuda.synchronize()
                    # get_key_ed = time.monotonic()
                    # with open("prefetch_profile/get_token_profile.txt", 'a') as f:
                    #     f.write(f"get time: {get_key_ed - get_key_st:.8f} s, token_id len: {len(token_id)}, device: {device_key}\n")
                if need_value:
                    ...
                    # torch.cuda.synchronize()
                    # get_value_st = time.monotonic()
                    value_buffer[mask] = chunk.get_value(token_id,is_prefetching=True)
                    # torch.cuda.synchronize()
                    # get_value_ed = time.monotonic()
                    # with open("prefetch_profile/get_token_profile.txt", 'a') as f:
                    #     f.write(f"get time: {get_value_ed - get_value_st:.8f} s, token_id len: {len(token_id)}, device: {device_value}\n")
        if self.reorder is not None:
            return key_buffer,value_buffer, self.reverse_reorder[prefetch_tokenids]
        else:
            return key_buffer,value_buffer, prefetch_tokenids
...
class PrefixKV():
    ...
    def prefetch(self,pos_id=None,layer=None,k_v_num_tokens=None,time_budget=0.0):
        cache = self.layers[layer]
        key,value,tokenids = cache.prefetch(pos_id=pos_id,k_v_num_tokens=k_v_num_tokens,time_budget=time_budget)
...
class Pcache():
    ...
    def prefetch(self, prefix_id, pos_id=None, layer=None, k_v_num_tokens=None,time_budget=0.0):
        cache = self.cache[prefix_id]
        key, value, tokenids = cache.prefetch(pos_id, layer, k_v_num_tokens=k_v_num_tokens,time_budget=time_budget)
        return key, value, tokenids

    def prefetch_update(self, prefix_id, pos_id=None, chunk_ids=None, layer=None):
        # prefetch_update is used to prefetch the kv cache of the tokens in pos_id into higher level cache
        # it tries to fetch the pointed cache asynchronously
        cache = self.cache[prefix_id]
        return cache.prefetch_update(pos_id=pos_id, chunk_ids=chunk_ids, layer=layer)
    ...
    def get_hit_rate(self):
        ...
        print(f'gpu_hit:{gpu_hit},cpu_hit:{cpu_hit},disk_hit:{disk_hit},gpu_hit_rate:{gpu_hit/(gpu_hit+cpu_hit+disk_hit)},cpu_hit_rate:{cpu_hit/(gpu_hit+cpu_hit+disk_hit)},disk_hit_rate:{disk_hit/(gpu_hit+cpu_hit+disk_hit)}')
    ...
    def get_time(self):
        ...
        print(f'dt_gpu:{dt_gpu},dt_cpu:{dt_cpu},dt_disk:{dt_disk},control update time:{Pcache.control_update_time},gpu_rate:{dt_gpu/(dt_gpu+dt_cpu+dt_disk)},cpu_rate:{dt_cpu/(dt_gpu+dt_cpu+dt_disk)},disk_rate:{dt_disk/(dt_gpu+dt_cpu+dt_disk)}')
