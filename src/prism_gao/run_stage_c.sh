...
RUN_ROOT="${RUN_ROOT:-$ROOT/src/prism_gao/results/stage_c}"
...
case "$METHOD" in
  ...
  promixed)
    ...
    method_args=(
      ...
      --no-impress-value-ordered-prefetch
      --exact-layer-block-budget
      --no-impress-known-period-prefetch
    )
    ;;
  as_lru)
    if [[ "$SELECTOR_BACKEND" != "fp16" ]]; then
...
PYTHONPATH="$ROOT/src" \
PRISM_GAO_SAMPLE_OFFSET="$SAMPLE_OFFSET" \
timeout --kill-after=60s "${RUN_TIMEOUT_SECONDS}s" "$PYTHON" \
  -m prism_gao.run_fused_selector \
