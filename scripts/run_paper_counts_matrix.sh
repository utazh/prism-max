...
ROOT="${ROOT:-/home/panzihang/src/prism_paper_counts_20260803}"
PYTHON="${PYTHON:-/home/panzihang/venvs/vllm-stable/bin/python}"
ANALYSIS_PYTHON="${ANALYSIS_PYTHON:-/usr/bin/python3}"
GPU="${GPU:-3}"
RUN_ROOT="${RUN_ROOT:-$ROOT/results/qwen7_paper_counts_410_20260803}"
MODEL_PATH="${MODEL_PATH:-/data1/llm/Qwen/Qwen2.5-7B-Instruct}"
BUNDLE_DIR="${BUNDLE_DIR:-$ROOT/data/paper_task_bundles_100_110_120_80}"
...
"$PYTHON" - "$BUNDLE_DIR/metadata.json" "$BUNDLE_DIR" "$IMPRESS_REORDER" <<'PY'
import hashlib
import json
import sys
...
reorder_path = sys.argv[3]
expected = {"sst2": 100, "subj": 110, "trec": 120, "rte": 80}
if metadata.get("evaluation_requests_by_task") != expected:
    raise SystemExit(
        f"bundle request counts are {metadata.get('evaluation_requests_by_task')}; expected {expected}"
    )
if metadata.get("total_evaluation_requests") != sum(expected.values()):
    raise SystemExit("bundle total request count is not 410")
reorder_bytes = open(reorder_path, "rb").read()
if metadata.get("excluded_uids_manifest_sha256") != hashlib.sha256(reorder_bytes).hexdigest():
    raise SystemExit("bundle was not filtered against the active IMPRESS calibration manifest")
reorder = json.loads(reorder_bytes)
for task, count in expected.items():
    rows = [json.loads(line) for line in open(f"{bundle_dir}/{task}.jsonl", encoding="utf-8") if line.strip()]
    uids = {str(row["uid"]) for row in rows}
    history_uids = {str(uid) for uid in reorder["tasks"][task]["history_uids"]}
    if len(rows) != count or len(uids) != count:
        raise SystemExit(f"{task} does not contain {count} unique requests")
    if uids.intersection(history_uids):
        raise SystemExit(f"{task} evaluation requests overlap IMPRESS calibration")
PY
...
# Rotate method order across budgets to limit systematic warm-storage bias.
...
sha256sum \
  ...
  "$ROOT/scripts/build_paper_count_bundles.py" \
  "$ROOT/scripts/analyze_paper_matrix.py" \
  "$ROOT/scripts/run_paper_counts_matrix.sh" \
  "$ROOT/configs/"*.json \
  "$BUNDLE_DIR/metadata.json" \
  "$BUNDLE_DIR/"*.jsonl \
  "$CONTIG_KV_DIR/.contiguous_fuxian_complete" \
