...
ROOT="${ROOT:-/home/panzihang/src/prism_phase_b_rolling_20260730}"
...
GPU="${GPU:-0}"
RUN_ROOT="${RUN_ROOT:-$ROOT/results/rolling_single_request_20260730}"
...
write_hashes() {
  sha256sum \
    ...
    "$ROOT/scripts/analyze_rolling_pilot.py" \
    "$ROOT/scripts/run_rolling_single_request_screen.sh" \
    "$PLAN" \
    "$PROFILE" \
...
run_variant() {
  ...
  if [[ "$rolling" == "1" ]]; then
    rolling_args=(--impress-rolling-period-prefetch)
  fi
...
# Four diverse batch-size-1 requests gate the mechanism before the larger screen.
run_variant "pilot_leader_4" "sst2,subj,trec,rte" 1 0
run_variant "pilot_rolling_4" "sst2,subj,trec,rte" 1 1

set +e
"$PYTHON" "$ROOT/scripts/analyze_rolling_pilot.py" \
  --leader "$RUN_ROOT/pilot_leader_4" \
  --rolling "$RUN_ROOT/pilot_rolling_4" \
  --output "$RUN_ROOT/rolling_pilot_report.json" \
  >"$RUN_ROOT/rolling_pilot_report.log" 2>&1
pilot_status=$?
set -e
write_hashes
if [[ "$pilot_status" != "0" ]]; then
  touch "$RUN_ROOT/PILOT_GATE_FAIL"
  if [[ "${FORCE_SCREEN:-0}" != "1" ]]; then
    cat "$RUN_ROOT/rolling_pilot_report.json"
    exit "$pilot_status"
  fi
  touch "$RUN_ROOT/PILOT_EXPLORATORY_OVERRIDE"
else
  touch "$RUN_ROOT/PILOT_GATE_PASS"
fi

run_variant "screen_leader_a_32" "sst2,subj,trec,rte" 8 0
run_variant "screen_rolling_a_32" "sst2,subj,trec,rte" 8 1
run_variant "screen_rolling_b_32" "sst2,subj,trec,rte" 8 1
run_variant "screen_leader_b_32" "sst2,subj,trec,rte" 8 0

set +e
"$PYTHON" "$ROOT/scripts/analyze_abba_screen.py" \
  --reference-a "$RUN_ROOT/screen_leader_a_32" \
  --candidate-a "$RUN_ROOT/screen_rolling_a_32" \
  --candidate-b "$RUN_ROOT/screen_rolling_b_32" \
  --reference-b "$RUN_ROOT/screen_leader_b_32" \
  --candidate-name "rolling-period-source" \
  --feature-flag-metric "impress_rolling_period_prefetch" \
  --output "$RUN_ROOT/rolling_screen_report.json" \
  >"$RUN_ROOT/rolling_screen_report.log" 2>&1
analysis_status=$?
set -e
...
cat "$RUN_ROOT/rolling_screen_report.json"
