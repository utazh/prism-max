...
ROOT="${ROOT:-/home/panzihang/src/prism_phase_b_deferred_20260730}"
...
GPU="${GPU:-0}"
RUN_ROOT="${RUN_ROOT:-$ROOT/results/deferred_single_request_20260730}"
...
run_variant() {
  ...
  local deferred_args=(--no-impress-deferred-compute-timing)
  if [[ "$deferred" == "1" ]]; then
    deferred_args=(--impress-deferred-compute-timing)
  fi
...
run_variant "smoke_sync_1" "sst2" 1 0
run_variant "smoke_deferred_1" "sst2" 1 1

run_variant "screen_sync_a_32" "sst2,subj,trec,rte" 8 0
run_variant "screen_deferred_a_32" "sst2,subj,trec,rte" 8 1
run_variant "screen_deferred_b_32" "sst2,subj,trec,rte" 8 1
run_variant "screen_sync_b_32" "sst2,subj,trec,rte" 8 0

set +e
"$PYTHON" "$ROOT/scripts/analyze_abba_screen.py" \
  --reference-a "$RUN_ROOT/screen_sync_a_32" \
  --candidate-a "$RUN_ROOT/screen_deferred_a_32" \
  --candidate-b "$RUN_ROOT/screen_deferred_b_32" \
  --reference-b "$RUN_ROOT/screen_sync_b_32" \
  --candidate-name "deferred-compute-timing" \
  --expected-candidate-timing-samples 28 \
  --output "$RUN_ROOT/deferred_screen_report.json" \
  >"$RUN_ROOT/deferred_screen_report.log" 2>&1
analysis_status=$?
set -e
...
sha256sum \
  ...
  "$ROOT/scripts/run_deferred_single_request_screen.sh" \
  "$PLAN" \
  "$PROFILE" \
  "$FLEXGEN_ROOT/my_pcache_fast.py" \
  "$BUNDLE_DIR/metadata.json" \
...
cat "$RUN_ROOT/deferred_screen_report.json"
